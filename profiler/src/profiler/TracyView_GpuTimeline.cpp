...
bool View::DrawGpu( const TimelineContext& ctx, const GpuCtxData& gpu, const std::vector<GpuLaneDraw>& lanes, int& offset )
{
    ...
    for( auto& lane : lanes )
    {
        ...
        const bool markers = lane.markerBegin != lane.markerEnd;
        const int markerRows = markers ? 1 : 0;
        if( markers ) DrawGpuMarkers( ctx, td->second.markers, lane.markerBegin, lane.markerEnd, offset, begin, drift );
...
void View::DrawGpuMarkers( const TimelineContext& ctx, const Vector<short_ptr<GpuMarkerData>>& vec, uint32_t first, uint32_t last, int offset, int64_t begin, int drift )
{
    ...
    // [first, last) was resolved in the preprocess pass, under the same data lock this draw runs under.
    auto it = vec.begin() + first;
    const auto zitend = vec.begin() + last;

    if( wpos.y + offset + ty < ctx.yMin || wpos.y + offset > ctx.yMax ) return;
